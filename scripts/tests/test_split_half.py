...
def test_data(root_dir):
    # test data is the 'simdata' produced by the splithalf package for R:
    # https://github.com/sdparsons/splithalf
    test_data = pd.read_csv(
        f'{root_dir}/test_split_half_data/test_split_half_data.csv')
...
def single_condition_data(test_data):
    ...
    test_data = [test_data[test_data.participant_number ==
                           p].RT for p in participant_list]
    return test_data
...
def test_single_condition_reliability(single_condition_data):
    result = scripts.reliability.reliability.split_half(
        single_condition_data, None, 5000)
    # comparison values derived from the splithalf package for R:
    # https://github.com/sdparsons/splithalf
...
def test_condition_difference_reliability(two_condition_data):
    result = scripts.reliability.reliability.split_half(
        two_condition_data[0], two_condition_data[1], 5000)
    # comparison values derived from the splithalf package for R:
    # https://github.com/sdparsons/splithalf
