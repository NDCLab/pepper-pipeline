...
def spearman_brown(correlation_coefficient):
    """Apply Spearman-Brown Correction, to convert a Pearson correlation to measure of reliability.
    Parameters
    ----------
...
def split_half(data_group_one, data_group_two=None, number_permutations=5000):
    """Compute split-half reliability for either a single measure, or a difference between two measures.
    Parameters
    ----------
    data_group_one: list
                    A list of 1D numpy arrays. Each array represents data from one group.
    ...
    ----------
    namedtuple
        For both correlation and reliability, the mean, lower, and upper 95% CI of the distribution across permutations
    """
    ...
    if compute_difference and (len(data_group_one) != len(data_group_two)):
        raise ValueError('When computing a difference, data_group_one and data_group_two must be the same length')
    ...
    for n in range(number_permutations):
        ...
        corr_value = np.corrcoef(mean_by_split, rowvar=False)[0, 1]  # [0,1] to retain 1 value from sym matrix
