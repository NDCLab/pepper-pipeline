...
def faster_bad_channels(epochs, picks=None, thres=3, use_metrics=None):
    ...
    metrics = {
        ...
        'correlation': lambda x: np.nanmean(np.ma.masked_array(
                                                np.corrcoef(x),
    ...
    for m in use_metrics:
        s = metrics[m](data)
        b = [epochs.ch_names[picks[i]] for i in _find_outliers(s, thres)]
...
def faster_bad_components(ica, epochs, thres=3, use_metrics=None):
    ...
    metrics = {
        ...
        'kurtosis': lambda x: kurtosis(np.dot(
                                            x.mixing_matrix_.T,
...
def faster_bad_channels_in_epochs(epochs, picks=None, thres=3,
    ...
    for m in use_metrics:
        ...
        for i, s in enumerate(s_epochs):
            b = [epochs.ch_names[picks[j]] for j in _find_outliers(s, thres)]
